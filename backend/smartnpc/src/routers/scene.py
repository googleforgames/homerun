...
cached_conv_example = CacheFactory(config).get_cache(
    "conv_example"
)
...
def ensure_json_dict(input_text:any) -> dict:
    """
    ...
    if isinstance(input_text, dict):
        ...
        try:
            return ensure_json_dict(json.loads(input_text))
        except json.JSONDecodeError as e:
            logging.error("* Unable to decode json string:%s", f"{e}")
            return {}
...
def chat(req:NPCSceneConversationRequest) -> NPCSceneConversationResponse:
    ...
    #       Because all charachter information,
#           including the main character inforamtion
...
""", req.input)
    ...
    logging.info({"message":f"* answer.candidates={answer.candidates}"}) # pylint: disable=f-string-without-interpolation
    if answer is not None:
        json_answer_text = answer.candidates[0].content.parts[0].text.replace(
            "```json",
            "").replace(
                "```",
                ""
        ...
        logging.info("json_answer_text=%s", json_answer_text)
        ...
        occurence = substring_occurence(
            test_str=json_answer_text,
            test_sub="[CHAR("
        )
        ...
        json_answer_text = json_answer_text.replace("\\n", "")
        response_text = json_answer_text
        ...
        logging.error({"message":f"No llm response."}) # pylint: disable=f-string-without-interpolation
        raise HTTPException(status_code=400,
                    detail="Error: no llm response")
