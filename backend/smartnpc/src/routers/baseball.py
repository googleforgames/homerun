...
def get_suggestions(req:GetSuggestionsRequest) -> NPCSceneConversationResponse:
    ...
    if player_lineup is not None:
        logging.info("** player_lineup:%s", json.dumps(player_lineup))
    else:
        logging.info("** player_lineup is None")
    if computer_lineup is not None:
        logging.info("** computer_lineup:%s", json.dumps(computer_lineup))
    else:
        logging.info("** computer_lineup is None")
...
async def response_handler(text:str,
                           ws:WebSocket,
                           req:NPCSceneConversationRequest) -> None:
    """
