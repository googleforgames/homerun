...
class RedisCacheWrapper:
    ...
    def keys(self) -> list[str]:
        ...
        for key in self.__client.keys():
            results.append(
                f"""{loads(key)}:{loads(self.__client.get(loads(key))) if self.__client.get(loads(key)) is not None else ""}""" # pylint: disable=line-too-long
            )
        return results
...
class CacheWrapper:
    ...
    def __init__(self, config, prefix):
        ...
        self.__config = config # pylint: disable=unused-private-member
        self.__cache = {}
    ...
    def get(self, key:str) -> any:
        ...
        if self.__key(key) in self.__cache.keys(): # pylint: disable=consider-iterating-dictionary
    ...
    def keys(self) -> any:
        ...
        """
        results = []
        for key in self.__cache.keys(): # pylint: disable=consider-iterating-dictionary, consider-using-dict-items
...
class CacheFactory:
    ...
    def get_cache(self,
                  key_prefix:str
                ) -> Union[CacheWrapper, RedisCacheWrapper]:
        if self.__config["gcp"]["use-cache-server"] == "True":
            return RedisCacheWrapper(self.__config, key_prefix)
        else:
            global in_memory_cache # pylint: disable=global-variable-not-assigned
            if key_prefix in in_memory_cache.keys(): # pylint: disable=consider-iterating-dictionary, consider-using-dict-item
