...
This module provides a class for managing prompts, including retrieving
prompt templates, constructing prompts by filling in placeholders, and
caching prompt templates for efficient access.
"""
import re
import os
import logging
...
from models.npc import NPCInfoRequest
...
class PromptManager():
    ...
    def __extract_placeholders(self, prompt:str):
        ...
        Args:
            prompt (str): The prompt string containing
    ...
    def construct_prompt(self, prompt_id:str,
        ...
        """
        Get prompt template from the database,
        parse the template.
        Fill in placeholders with corresponding
        ...
        Returns:
            Prompt template.
        """
        logging.info("construct_prompt: %s | %s",
                     scene_id,
                     prompt_id)
        ...
        if prompt_template:
            ...
            logging.error(
                "construct_prompt():No Prompte Template found:%s | %s",
                          scene_id,
                          prompt_id)
    ...
    def __update_place_holders(self,
        ...
        npc_in_the_scene = ",".join(
            [id for id in scene.npc_ids]
        ) if scene.npc_ids is not None else ""
        npc_manager = NPCManager(config=self._config)
        for place_holder in prompt_template.place_holders:
            match place_holder:
                case "NON_PLAYER_CHARACTERS":
                    prompt_template.prompt_template = prompt_template.prompt_template.replace( # pylint: disable=line-too-long
                ...
                case "CHARACTER_BACKGROUND":
                    npcs = []
                    for npcid in scene.npc_ids if scene.npc_ids is not None else []: # pylint: disable=line-too-long
                ...
                case _:
                    if "NPC:" in place_holder:
                        ...
                        logging.warning({
                            "message": f"Placeholder: {place_holder} not implemented." # pylint: disable=line-too-long
                        })
        return prompt_template
