...
This module provides a class for managing conversations, including retrieving
conversation examples and handling multi-turn interactions with an LLM.
"""
...
class ConversationManager():
    ...
    def __init__(self, config:dict):
        ...
        """
        self._cache = CacheFactory(config=config).get_cache(
            "conversations"
        )
        self._conversation_history = CacheFactory(config=config).get_cache(
    ...
    def __format_conversation_history_cache_key(
        ...
        Returns:
            Formatted cache key.
        """
        return f"{game_id}/{scene_id}/{player_id}/{session_id}"
...
.
        ...
        logging.info("* conversationManager:chat|result %s :", result)
